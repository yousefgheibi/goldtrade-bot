...
import { ADMIN_CHAT_ID, USERS_FILE, APPROVAL_DURATION_DAYS } from "../config.js";
...
export function handleApprove(msg, match) {
  ...
  user.approvedUntil = new Date(Date.now() + APPROVAL_DURATION_DAYS * 24*60*60*1000);
  fs.writeFileSync(USERS_FILE, JSON.stringify(users, null, 2));

  bot.sendMessage(chatId, `✅ کاربر ${user.name} تأیید شد.`);
  bot.sendMessage(targetChatId, "✅ دسترسی شما تا ۳۰ روز آینده فعال شد. حالا می‌توانید از ربات استفاده کنید.");
}
...
function registerUser(chatId, name) {
  ...
  if (!user) {
    ...
    bot.sendMessage(ADMIN_CHAT_ID, `📢 کاربر جدید درخواست دسترسی داده:\n👤 ${name}\n🆔 ${chatId}\n\nبرای تأیید تا ۳۰ روز آینده دستور زیر را ارسال کن:\n/approve ${chatId}`);
    bot.sendMessage(chatId, "⏳ درخواست شما برای استفاده از ربات ثبت شد. لطفاً منتظر تأیید ادمین باشید.");
  } else {
    if (user.status === "approved") sendMainMenu(chatId);
    else if (user.status === "expired") bot.sendMessage(chatId, "❌ مدت اعتبار شما منقضی شده است. لطفاً منتظر تأیید مجدد باشید.");
